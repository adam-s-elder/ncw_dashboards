...
This only needs to be done once. After you run this chunk the first time or if you have already set the key in the past, comment out the code in this chunk. 
```{r}
census_api_key("YOUR API KEY", overwrite=TRUE, install=TRUE) # replace "YOUR API KEY" with the key you get in the email after requesting it
...
perc_uninsured_file <- here("ncw_data", "cleaned_data", "insurance_data.csv")
```
...
write_csv(all_data %>% filter(Indicator == "perc_uninsured"), perc_uninsured_file)
```
